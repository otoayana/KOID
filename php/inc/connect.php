...
most of this code has been lifted from miiverse world by pf2m (because i don't know squat about sql lol), which is licensed under an mit license.
...
function ruBlocked($source, $target, $either = false) {
    global $db;
    if($either) {
...
function getAvatar($avatar, $has_mh) {
    if($has_mh == 0) {
        if(empty($avatar)) {
                        ...
                        <div class="window-body">
                            <form method="post" action="/users/<?=htmlspecialchars(urlencode($row['username']))?>/blacklist.delete.json">
                                ...
                                <div class="form-buttons">
                                    <input type="button" class="modal-close-button gray-button" value="Cancel">
...
if(empty($_SESSION['username']) && !empty($_COOKIE['mwauth'])) {
    ...
    $stmt->bind_param('s', $_COOKIE['koidauth']);
    $stmt->execute();
    if($stmt->error) {
        goto skip;
    }
    ...
    $stmt = $db->prepare('SELECT username, avatar, level FROM users WHERE id = ?');
